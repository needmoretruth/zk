...
/// `p = 2^64 − 2^32 + 1`. The linear decomposition works over any ring (§4.1); a 64-bit prime field
/// keeps shares cheap and holds every museum statement without wrap-around.
...
impl Fp {
    ...
    /// the only way bytes from a tape or a proof become an element, so no value has two encodings.
