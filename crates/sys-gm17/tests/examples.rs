//! Every statement through the shared harness: ark-gm17's verifier accepts the honest proof, every
//! attack fails, and no secret appears in the proof bytes.

#![allow(clippy::unwrap_used, clippy::expect_used)]

use sys_gm17::{Gm17, TAMPER_OFFSET};
use zk_core::{AttackKind, Control, ExampleId, ProofSystem, RunOptions, SecretScan, Verdict};

const SEED: [u8; 32] = [42; 32];

/// arkworks' compressed encoding: two 48-byte G1 points and one 96-byte G2 point.
const PROOF_BYTES: u64 = 192;

fn check(example: ExampleId) {
    let options = RunOptions { seed: Some(SEED), attacks: true };
    let report = Gm17.run(example, &options, &Control::new()).unwrap();
