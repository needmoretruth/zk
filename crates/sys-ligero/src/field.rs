...
/// `p = 2^64 − 2^32 + 1`. Ligero needs two things from its field: a power-of-two subgroup large
/// enough to Reed–Solomon-encode rows with the NTT (Goldilocks has one of order 2^32), and a size
/// that makes a random combination's error `1/|F|` small (about 2^-64 per repetition).
...
impl Fp {
    ...
    /// the only way bytes from randomness or a proof become an element, so no value has two encodings.
