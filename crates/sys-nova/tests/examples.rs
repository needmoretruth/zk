//! Every statement through the shared harness: Nova's compressed verifier accepts the honest chain,
//! every attack fails, and no secret appears in the proof bytes.

#![allow(clippy::unwrap_used, clippy::expect_used)]

use sys_nova::{Nova, TAMPER_OFFSET};
...
fn check(example: ExampleId) {
    ...
    let report = Nova.run(example, &options, &Control::new()).unwrap();
