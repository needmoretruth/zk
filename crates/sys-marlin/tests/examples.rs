//! Every statement through the shared harness: ark-marlin's verifier accepts the honest proof, every
//! attack fails, and no secret appears in the proof bytes.

#![allow(clippy::unwrap_used, clippy::expect_used)]

use sys_marlin::{Marlin, SRS_BYTES_COUNT, TAMPER_OFFSET};
use zk_core::{
...
/// arkworks' compressed encoding, the same for every circuit: nine KZG commitments (two with a shifted
/// twin) in 569 bytes, seven 32-byte evaluations behind a length, three empty prover messages, and
/// two opening proofs (one per query point), one of them carrying a hiding scalar.
const PROOF_BYTES: u64 = 951;

fn check(example: ExampleId) {
    let options = RunOptions { seed: Some(SEED), attacks: true };
    let report = Marlin.run(example, &options, &Control::new()).unwrap();
    ...
    println!(
        ...
        report.proof_bytes,
        report.setup_bytes.unwrap(),
        count(SRS_BYTES_COUNT),
        outcomes.join(" / "),
        report.secret_scan,
    );
    ...
    // The flipped byte sits in a scalar that still decodes, so the refusal is the verifier's own.
    assert_eq!(flip.outcome, AttackOutcome::Rejected);
    assert!(count(SRS_BYTES_COUNT) > 0);
