...
///
/// ark-marlin indexes and proves by replaying a circuit's `generate_constraints` against a fresh
/// `ConstraintSystem`. Replaying the lowered R1CS unchanged, rather than rewriting the statement
/// with arkworks gadgets, keeps the constraints Marlin proves identical to those every other R1CS
/// system in the museum proves. Every row is handed over as concrete linear combinations, never as
/// symbolic ones, so the outlining ark-marlin asks arkworks for (`OptimizationGoal::Weight`) finds
/// nothing to outline and the index has exactly the museum's rows; ark-marlin then pads the public
/// input to a power of two and the matrices to a square itself.
...
impl<'a> Synthesis<'a> {
    /// For indexing: no witness exists, and arkworks' setup mode never asks for a value.
