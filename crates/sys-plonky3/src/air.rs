//! The lowered circuit as a Plonky3 AIR.

use p3_air::{Air, AirBuilder, BaseAir, WindowAccess};
use zk_circuit::lower::wide_air::{AirConstraint, WideAir};

use crate::field::{BabyBear, Val};
...
/// constraint reads the next row, so the prover never opens the trace at `g·ζ`.
pub(crate) struct CircuitAir {
    wide: WideAir<BabyBear>,
}

impl CircuitAir {
    /// Wraps a lowered circuit.
    pub(crate) fn new(wide: WideAir<BabyBear>) -> Self {
        Self { wide }
    }

    /// The lowered circuit, for building the trace row and reporting its shape.
    pub(crate) fn wide(&self) -> &WideAir<BabyBear> {
        &self.wide
    }
...
fn folded<AB: AirBuilder<F = Val>>(
    constraint: &AirConstraint<BabyBear>,
    row: &[AB::Var],
) -> AB::Expr {
