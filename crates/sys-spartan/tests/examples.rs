//! Every statement through the shared harness: Spartan's verifier accepts the honest proof, every
//! attack fails, and no secret appears in the proof bytes under any encoding Spartan uses.

#![allow(clippy::unwrap_used, clippy::expect_used)]

use sys_spartan::{Spartan, TAMPER_OFFSET};
...
fn check(example: ExampleId) {
    ...
    let report = Spartan.run(example, &options, &Control::new()).unwrap();
